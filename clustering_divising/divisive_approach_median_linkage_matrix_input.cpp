...
int main() {
    ...
    */
    ...
    Point centerA("m1",pair<double,double>(0,0)); 
    Point centerB("m2",pair<double,double>(0,0));
    ...
    for( auto it = clusterB.begin() ; it != clusterB.end() ; it++ ) {
        ...
        for(int i = 0 ; i < num_data ; i++ ) {
            ...
            distances.push_back(dist_matrix[counter][i]);
        }

        double delta = getAverage(distances);
    ...
    while ( highest_delta > 0 ) {
        ...
        for( auto it = clusterB.begin() ; it != clusterB.end() ; it++ ) {
            ...
            vector<double> distancesA;
            for(int i = 0 ; i < num_data ; i++ ) {
                for(auto itt: clusterA)
                    distancesA.push_back(dist_matrix[it->id][itt.id]);
            }

            vector<double> distancesB;
            for(int i = 0 ; i < num_data ; i++ ) {
                if ( i == counter ) continue;
                for(auto itt: tempB)
                    distancesB.push_back(dist_matrix[it->id][itt.id]);
            }


            double distA = getAverage(distancesA);
            double distB = getAverage(distancesB);
